...
export const Knight = () => {
    ...
    const setHint = (coords: [number, number]) => {
        ...
        const newField = [...field];
        for (let move of legitMoves) {
            if (newField[row + move[0]] && newField[row + move[0]][col + move[1]] && !newField[row + move[0]][col + move[1]].value) {
            ...
            <div>
                <div>
                    ...
                    <div>
                        ...
                        <button onClick={onCancel} disabled={!history.length}>Отменить ход</button>
                    </div>
                </div>
